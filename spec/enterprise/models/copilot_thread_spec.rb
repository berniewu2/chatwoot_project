...
RSpec.describe CopilotThread, type: :model do
  ...
  describe '#push_event_data' do
    ...
    let(:copilot_thread) { create(:ai_agent_copilot_thread, account: account, user: user, topic: topic, title: 'Test Thread') }
  ...
  describe '#previous_history' do
    ...
    context 'when there are messages in the thread' do
      before do
        ...
        create(:ai_agent_copilot_message, copilot_thread: copilot_thread, message_type: 'topic_thinking', message: { 'content' => 'Thinking...' })
        create(:ai_agent_copilot_message, copilot_thread: copilot_thread, message_type: 'topic', message: { 'content' => 'Topic message' })
      end
